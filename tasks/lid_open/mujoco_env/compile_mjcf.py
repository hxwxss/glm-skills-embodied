...
Compiles spec/scene_spec.json into generated/lid_scene.xml: floor, lab
table, static decorations, and the hinged-lid storage box (via the shared
ir_emitter, the same code path the robosuite arena uses).

The standalone scene is the artifact for the M1 physics self-check
(settle / no-penetration / hinge flip-and-hold proof).
...
def compile_xml(spec):
    ...
    for obj in spec["objects"]:
        if obj["physics"] == "static":
            ...
            ir_emitter.emit_lid_box(world, obj)
...
def main():
    ...
    out_path = os.path.join(args.out, "lid_scene.xml")
